...
class CategoryWithBarbersBlocBuilder extends StatelessWidget {
    ...
    return BlocBuilder<CategoryCubit, CategoryState>(
      ...
      builder: (context, state) {
        return state.maybeMap(
          categoryLoading: (_) => setupLoading(),
          categorySuccess: (successState) =>
              setupSuccess(successState.serviceResponseModel, context),
          categoryError: (_) => setupError(),
  ...
  Widget setupSuccess(
      CategoriesListResponseModel response, BuildContext context) {
    ...
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          CategoryBarberListView(barberDataList: categoryData ?? []),
        ],
      ),
    );
  }
