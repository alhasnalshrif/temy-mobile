    ...
    try {
      ...
      if (payloadMap.containsKey('exp')) {
        ...
        if (timeUntilExpiry.inHours < 1) {
          debugPrint(
            '⚠️ AuthService: Token expires soon (${timeUntilExpiry.inMinutes} minutes)',
          );
        }
      }
      ...
      if (payloadMap.containsKey('userId') ||
          ...
          payloadMap.containsKey('sub')) {
        final userId =
            payloadMap['userId'] ?? payloadMap['id'] ?? payloadMap['sub'];
        debugPrint('✅ AuthService: Token contains user ID: $userId');

        // Store/update user ID if not already stored
        final storedUserId = await SharedPrefHelper.getSecuredString(
